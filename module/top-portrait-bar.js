export class TopPortraitBar {
    ...
    static async render(actor) {
        ...
        if (portraitBar.find('#actor-portrait-' + actor.id).length === 0) {
            var $portraitDiv = $("<div id='actor-portrait-" + actor.id + "' class='portrait " + partyHudType + "''><div class='barbox " + partyHudType + "'><span class='name'>" + actor.name + "</span> <div class='damagebar'><div class='background'></div> <div class='damage'></div><span class='life'>10/10</span></div></div><div class='buffbox flexrow " + partyHudType + "'></div><img src='" + (useTokenImages ? (actor.prototypeToken?.texture?.src || actor.img) : actor.img) + "'><div class='overlay'></div></div>")
        ...
        let items = actor.data.items.filter(o => (o.type === "buff") && getProperty(o.data, "data.active") === true).sort((a, b) => {
