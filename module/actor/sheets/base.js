...
export class ActorSheetPF extends ActorSheet {
  ...
  async getData() {
    ...
    const sheetData = {
      ...
      isPlayerEditLocked: (this.document.system.lockEditingByPlayers || false) && !game.user.isGM,
      hasRace: false,
      config: CONFIG.D35E,
      useBGSkills: this.document.type === "character" && game.settings.get("D35E", "allowBackgroundSkills"),
      hideShortDescriptions: game.settings.get("D35E", "hideSpells"),
      spellFailure: this.document.spellFailure,
      isGM: game.user.isGM,
      race: this.document.race != null ? duplicate(this.document.race) : null,
    };
    ...
    for (let [s, skl] of Object.entries(sheetData.actor.system.skills)) {
      ...
      if (s === "jmp") {
        if (sheetData.actor.system.attributes.speed.land.total < sheetData.actor.system.attributes.speed.land.base) {
          let value =
            6 *
            Math.floor(
              (sheetData.actor.system.attributes.speed.land.total - sheetData.actor.system.attributes.speed.land.base) / 10
            );
          skl.sourceDetails.push({
  ...
  _filterItems(items, filters) {
    return items.filter((item) => {
      ...
      if (filters.has("prepared")) {
        ...
        if (this.actor.system.type === "npc") return true;
  ...
  async _onSpellPrepareSpecialUses(event) {
    ...
    const itemId = $(event.currentTarget).parents(".item").attr("data-item-id");
    const item = duplicate(this.actor.items.get(itemId).data);
    delete item._id;
  ...
  async _onSpellAddMetamagic(event) {
    event.preventDefault();
    const itemId = $(event.currentTarget).parents(".item").attr("data-item-id");
    const newSpell = duplicate(this.actor.items.get(itemId).data.toObject(false));
    delete newSpell._id;
  ...
  async importItem(itemData, dataType) {
    if (itemData.type === "spell" && this.actor.system.type === "trap") {
    ...
    return this.actor.createEmbeddedDocuments("Item", [ itemData, { dataType: dataType });
  }
  ...
  async _openCompendiumPack(event) {
    ...
    if (pack.startsWith("browser")) {
      ...
      if (this.document.race !== null) {
        this.document.race.sheet.render(true);
      }
    } else if (pack === "actor-first-class") {
      this.document.items.find((o) => o.type === "class").sheet.render(true);
    }
  }
